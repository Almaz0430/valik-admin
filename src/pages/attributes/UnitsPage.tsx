...
import { brandUnitService, type Unit } from '../../features/attributes';
    ...
    <div className="space-y-6 pb-16 lg:pb-0">
      {isStandalone && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Управление единицами измерения</h1>
            <p className="mt-2 text-sm text-slate-500 font-medium">Всего единиц: {totalUnits}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        {!isStandalone && (
          <div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Единицы измерения</h2>
            <p className="mt-1 text-sm text-slate-500 font-medium">Всего единиц: {totalUnits}</p>
          </div>
        )}
      ...
      <div className="bg-white shadow-[0_2px_12px_-4px_rgba(0,0,0,0.08)] ring-1 ring-slate-200/50 rounded-2xl overflow-hidden box-border">
        ...
        {!isLoading && !error && (
          <div className="divide-y divide-gray-200">
            ...
            {units.map((unit) => (
              <div key={unit.id} className="px-4 sm:px-6 py-4 hover:bg-slate-50 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 custom-scrollbar">
          ...
          <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] ring-1 ring-slate-200/50 rounded-3xl overflow-visible flex flex-col transform transition-all z-[110]">
            ...
            <div className="flex justify-between items-start p-6 border-b border-slate-100/80 bg-white/50 rounded-t-3xl">
              <div>
                <h3 className="text-xl font-bold text-slate-900 tracking-tight">Создание единицы измерения</h3>
            ...
            <div className="p-6">
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Название (сокращение) *</label>
                  <input
                    type="text"
                    value={newUnitTitle}
